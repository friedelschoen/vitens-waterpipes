# Vitens Waterpipes -- Control and Dashboard for Waterpipes-Model

## What you need

-   The Raspberry Pi runs a daemon which constantly pushes sensor and valve states to the MQTT-broker.
    A broker accepts clients which can subscribe to a certain topic (message type) or publish a message with a topic.
-   The inserter subscribes to the sensordata, calculates predictions and stores them to the database. The database is SQLite3.
-   The dashboard
...
```json
{
    "raspberrypi": {
        "sensor.flow0": {
            ...
            "ae": [{ "timestamp": 1734600123.4, "value": 1.1, "unit": "L/min" }]
        }
    }
}
```
